...
export function injectTranslations(locale: string, translations: Array<LocalizedStringPayload | null | undefined>) {
  ...
  translations.forEach((item) => {
    ...
    let value = parsedContent?.[locale];
    if (value === undefined || value === null || value === '') {
      if (locale === 'vi') value = parsedContent?.vn;
      if (locale === 'vn') value = parsedContent?.vi;
    }
    if (value === undefined || value === null || value === '') {
      value = parsedContent?.en || parsedContent?.zh || Object.values(parsedContent || {}).find(v => typeof v === 'string' && !!v) || '';
    }
