...
export default async function RootPage() {
  ...
  try {
    ...
    console.error('[RootPage] Failed to fetch default language from DB:', e);
  }
  ...
  try {
    ...
    acceptLang = rawAccept?.split(',')[0].split('-')[0].toLowerCase();
  } catch (err) {
    console.error('[RootPage Debug] Accept-Language check failed:', err);
  }

  if (acceptLang && locales.includes(acceptLang)) {
