...
const EditorHeader = memo(({
  ...
  return (
    <div className="flex items-center justify-between sticky top-[-40px] -mt-10 z-50 bg-card/90 backdrop-blur-xl py-3 border border-border/40 shadow-[0_20px_50px_rgba(0,0,0,0.12)] rounded-2xl px-6 relative mb-8 w-full gap-4">
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <h2 className="text-xl font-headline font-bold text-foreground tracking-tight flex items-center gap-2">
          ...
          <span>{isEditing ? '修改产品' : '创建产品'}</span>
        </h2>
      </div>

      {/* 中间表单区域：资源归属分类 + 资产唯一标识 */}
      ...
      <div className="flex items-center gap-4 shrink-0">
        ...
        <Button
          onClick={onSave}
          disabled={isSaving}
          className="rounded-xl h-10 px-6 text-xs font-bold uppercase tracking-widest gap-2 shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all shrink-0"
        >
