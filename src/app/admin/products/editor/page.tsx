...
function ProductEditorContent() {
  ...
  const handleSave = () => {
    ...
    toast({ title: "产品已保存", description: "系统已智能合并多语言冗余项。" });
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-border/40 shadow-sm space-y-8">
            <div className="space-y-4">
              ...
              <Select value={formData.status} onValueChange={(v: 'published'|'draft') => setFormData({...formData, status: v})}>
                <SelectTrigger className="h-12 rounded-xl bg-muted/20 border-transparent transition-all hover:bg-muted/30"><SelectValue /></SelectTrigger>
            ...
            <div className="space-y-4 border-t pt-8">
              <div className="space-y-2"><Label className="text-[10px] font-bold uppercase text-primary">产品唯一 ID</Label><Input disabled={isEditing} value={formData.id} onChange={e => setFormData({...formData, id: e.target.value})} className="h-12 rounded-xl bg-muted/10 border-none font-mono text-xs" /></div>
              <div className="space-y-2"><Label className="text-[10px] font-bold uppercase text-primary">所属产品分类</Label><Select value={formData.categoryId} onValueChange={v => { setFormData({...formData, categoryId: v}); if(!isEditing) generateAutoId(v); }}><SelectTrigger className="h-12 rounded-xl bg-muted/20 border-transparent hover:bg-muted/30"><SelectValue placeholder="选择产品分类..." /></SelectTrigger><SelectContent className="rounded-xl">{categories?.map(c => <SelectItem key={c.id} value={c.id} className="text-xs">{getCatName(c.id)}</SelectItem>)}</SelectContent></Select></div>
            </div>
            <div className="space-y-6 pt-8 border-t">
              <div className="flex items-center justify-between"><Label className="text-[10px] font-bold uppercase text-primary flex items-center gap-2"><ImageIcon className="h-3 w-3" /> 产品主图</Label><button onClick={() => openPicker('main')} className="text-[9px] font-bold text-primary uppercase hover:underline">从图库挑选</button></div>
              <div className="space-y-4">
                <div className="relative aspect-square rounded-[2rem] bg-muted/20 border-2 border-dashed border-border/60 overflow-hidden flex items-center justify-center group transition-all hover:border-primary/40 hover:bg-muted/30 cursor-pointer" onClick={() => !formData.mainImageUrl && fileInputRef.current?.click()}>
                  {formData.mainImageUrl ? (<><Image src={formData.mainImageUrl} alt="Main" fill className="object-contain p-6 group-hover:scale-105 transition-transform duration-700" /><div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Button variant="destructive" size="sm" className="rounded-full h-10 w-10 p-0 shadow-2xl" onClick={(e) => { e.stopPropagation(); setFormData({...formData, mainImageUrl: ''}); }}><X className="h-5 w-5" /></Button></div></>) : (<div className="text-center space-y-3 opacity-40 group-hover:opacity-100 transition-opacity"><div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto text-primary"><Upload className="h-6 w-6" /></div><p className="text-[10px] font-bold uppercase tracking-tighter">点击上传或拖拽图片</p></div>)}
                  <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageUpload} />
                </div>
                <div className="space-y-2"><Label className="text-[9px] font-bold uppercase text-muted-foreground ml-1">上传到图库分类</Label><Select value={uploadGalleryCatId} onValueChange={setUploadGalleryCatId}><SelectTrigger className="h-10 rounded-xl bg-muted/10 border-transparent text-[10px]"><SelectValue placeholder="选择目标分类" /></SelectTrigger><SelectContent className="rounded-xl">{galleryCategoryTree.map(cat => (<SelectItem key={cat.id} value={cat.id} className="text-xs"><span style={{ paddingLeft: `${cat.depth * 0.5}rem` }} className="flex items-center">{cat.depth > 0 && <span className="mr-1.5 opacity-30">·</span>}{cat.name}</span></SelectItem>))}</SelectContent></Select></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {aiConfig?.isEnabled && (
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          className="h-6 text-[8px] gap-1 font-bold text-accent"
                          onClick={() => handleAiTranslateField(formData.nameZh, 'name')}
                          disabled={isAiProcessing || !formData.nameZh}
                        >
              ...
              <div className="bg-white p-10 rounded-[2.5rem] border border-border/40 shadow-sm space-y-8">
                <div className="flex items-center justify-between border-b border-border/40 pb-6">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">根据产品线创建不同的参数分组。</p>
                  </div>
                            ...
                            <div className="md:col-span-5 space-y-4">
                              <div className="space-y-1.5">
                                <div className="flex items-center justify-between">
                                  ...
                                  {aiConfig?.isEnabled && (
                                    <button onClick={() => handleAiTranslateSpec(gIdx, iIdx, item.labelZh, 'label')} className="text-accent hover:scale-110 transition-transform"><Sparkles className="h-3 w-3" /></button>
                                  )}
                                </div>
                            ...
                            <div className="md:col-span-6 space-y-4">
                              <div className="space-y-1.5">
                                <div className="flex items-center justify-between">
                                  ...
                                  {aiConfig?.isEnabled && (
                                    <button onClick={() => handleAiTranslateSpec(gIdx, iIdx, item.valueZh, 'value')} className="text-accent hover:scale-110 transition-transform"><Sparkles className="h-3 w-3" /></button>
                                  )}
                                </div>
            ...
            <TabsContent value="details" className="animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="bg-white p-10 rounded-[2.5rem] border border-border/40 shadow-sm space-y-8">
                <div className="flex items-center justify-between border-b border-border/40 pb-6 mb-4">
                  <div className="flex items-center gap-3"><Info className="h-5 w-5 text-primary" /><h3 className="font-bold text-lg">产品详细图文介绍</h3></div>
                  {aiConfig?.isEnabled && (
                    <Button variant="outline" size="sm" onClick={() => handleAiTranslateField(formData.detailsZh, 'details')} disabled={isAiProcessing || !formData.detailsZh} className="rounded-xl gap-2 text-accent border-accent/20">
                      {isAiProcessing ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />}
                      AI 润色翻译
                    </Button>
                  )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-4"><Label className="text-[10px] font-bold uppercase text-primary tracking-widest">中文详细介绍 (ZH)</Label><Textarea placeholder="支持长篇幅内容录入..." value={formData.detailsZh} onChange={e => setFormData({...formData, detailsZh: e.target.value})} className="min-h-[500px] rounded-[2rem] p-6 bg-muted/5 border-border/40 resize-none" /></div>
                  <div className="space-y-4"><Label className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest">English Detailed Info (EN)</Label><Textarea placeholder="Support for long-form detailed information..." value={formData.detailsEn} onChange={e => setFormData({...formData, detailsEn: e.target.value})} className="min-h-[500px] rounded-[2rem] p-6 bg-muted/5 border-border/40 resize-none" /></div>
                </div>
              </div>
            </TabsContent>
            <TabsContent value="gallery" className="animate-in fade-in slide-in-from-right-4 duration-500">
               <div className="bg-white p-10 rounded-[2.5rem] border border-border/40 shadow-sm space-y-8">
                 <div className="flex justify-between items-center border-b border-border/40 pb-6 mb-6">
                   <div className="space-y-1"><h3 className="text-xl font-bold text-primary flex items-center gap-2"><ImageIcon className="h-5 w-5" /> 副图库管理</h3><p className="text-xs text-muted-foreground">用于详情页相册展示的高清大图。</p></div>
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                   {formData.galleryUrls.map((url, idx) => (
                     <div key={idx} className="flex gap-4 p-5 bg-muted/10 rounded-[2rem] border border-border/20 group relative transition-all hover:bg-muted/20">
                       <div className="relative h-24 w-24 border border-border/40 rounded-2xl bg-white overflow-hidden shadow-sm shrink-0"><Image src={url} alt="Gallery Item" fill className="object-contain p-2" /></div>
                       <div className="flex-1 flex flex-col justify-between py-1">
                         <div className="space-y-2"><Label className="text-[8px] font-bold uppercase opacity-30">素材链接 URL</Label><Input value={url} onChange={e => { const g = [...formData.galleryUrls]; g[idx] = e.target.value; setFormData({...formData, galleryUrls: g}); }} className="h-9 text-[10px] bg-white rounded-lg border-none" /></div>
      ...
      <Dialog open={isPickerOpen} onOpenChange={setIsPickerOpen}>
        <DialogContent className="max-w-5xl p-0 rounded-[3rem] overflow-hidden flex flex-col h-[85vh] border-none shadow-2xl">
          <div className="bg-primary p-8 text-white"><DialogHeader><DialogTitle className="text-2xl font-bold flex items-center gap-3"><ImageIcon className="h-8 w-8" /> {pickerTarget === 'main' ? '选择产品主图' : '批量挑选副图库素材'}</DialogTitle><DialogDescription className="text-white/60 text-sm">从全球素材中心直接引用高清图片。</DialogDescription></DialogHeader></div>
          <div className="px-8 py-6 flex flex-col md:flex-row gap-4 bg-muted/20 border-b border-border/40"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="按标题搜索素材..." value={pickerSearch} onChange={e => setPickerSearch(e.target.value)} className="pl-10 h-12 bg-white rounded-xl border-none shadow-sm" /></div><Select value={pickerCat} onValueChange={setPickerCat}><SelectTrigger className="w-full md:w-64 h-12 rounded-xl bg-white border-none shadow-sm"><SelectValue placeholder="所有分类" /></SelectTrigger><SelectContent className="rounded-xl"><SelectItem value="all">所有分类</SelectItem>{galleryCategoryTree.map(cat => (<SelectItem key={cat.id} value={cat.id} className="text-xs"><span style={{ paddingLeft: `${cat.depth * 0.8}rem` }}>{cat.name}</span></SelectItem>))}</SelectContent></Select></div>
          <div className="flex-1 overflow-y-auto p-8 pt-6 bg-muted/5">
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {filteredAssets.map(a => (<div key={a.id} className={cn("group relative aspect-square bg-white rounded-2xl cursor-pointer overflow-hidden border-2 transition-all duration-300 hover:shadow-xl", selectedPickerUrls.has(a.url) ? "border-primary ring-4 ring-primary/20 scale-[0.98]" : "border-transparent hover:border-primary/20")} onClick={() => togglePickerSelection(a.url)}><Image src={a.url} alt={a.title} fill className="object-cover" />{selectedPickerUrls.has(a.url) && (<div className="absolute inset-0 bg-primary/20 backdrop-blur-[2px] flex items-center justify-center animate-in fade-in duration-300"><div className="bg-white rounded-full p-2 shadow-2xl"><Check className="text-primary h-8 w-8 stroke-[3]" /></div></div>)}<div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"><p className="text-[10px] text-white font-bold truncate">{a.title}</p></div></div>))}
            </div>
          </div>
          <DialogFooter className="p-8 bg-white border-t border-border/40 flex items-center justify-between"><div className="text-sm font-bold text-primary">已选中 <span className="text-2xl">{selectedPickerUrls.size}</span> 项素材</div><div className="flex gap-4"><Button variant="outline" onClick={() => setIsPickerOpen(false)} className="rounded-xl h-12 px-8 font-bold uppercase tracking-widest text-[10px]">取消</Button><Button onClick={handleConfirmPicker} disabled={selectedPickerUrls.size === 0} className="rounded-xl h-12 px-10 font-bold uppercase tracking-widest text-[10px] shadow-lg shadow-primary/20">确认添加</Button></div></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
