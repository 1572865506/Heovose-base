            ...
            <Table>
              <TableHeader className="bg-muted/30 border-b border-border/10">
                <TableRow className="hover:bg-transparent border-none">
                  <TableHead className="w-28 pl-10 font-bold uppercase text-[10px] tracking-[0.25em] text-muted-foreground/60 h-16">视觉</TableHead>
                  <TableHead className="font-bold uppercase text-[10px] tracking-[0.25em] text-muted-foreground/60 h-16">分类层级与命名</TableHead>
                  <TableHead className="font-bold uppercase text-[10px] tracking-[0.25em] text-muted-foreground/60 h-16">系统标识</TableHead>
                  <TableHead className="w-48 text-right pr-10 font-bold uppercase text-[10px] tracking-[0.25em] text-muted-foreground/60 h-16">操作中心</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isCatsLoading ? (
        ...
        <DialogContent className="rounded-[3rem] max-w-2xl p-0 overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] admin-interface-dark:shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border border-slate-200/50 admin-interface-dark:border-white/5 bg-card">
          ...
          <div className="p-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                ...
                <Select value={formData.parentId} onValueChange={v => setFormData({ ...formData, parentId: v })}>
                  <SelectTrigger className="h-14 rounded-2xl bg-muted/10 border border-slate-200 admin-interface-dark:border-white/10 text-sm font-bold shadow-inner">
              ...
              <div className="space-y-3">
                ...
                <Input
                  ...
                  className="h-14 rounded-2xl bg-muted/10 border border-slate-200 admin-interface-dark:border-white/10 font-mono text-sm font-black tracking-tighter placeholder:font-bold placeholder:opacity-20 shadow-inner"
                />
              </div>
            </div>
            ...
            <div className="space-y-6 pt-8 border-t border-border/40 admin-interface-dark:border-border/5">
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em]">AI智译</span>
                </ShinyButton>
              </div>
              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div className="space-y-2.5">
                    ...
                    <Input placeholder="高性能一体机" value={formData.nameZh} onChange={e => setFormData({ ...formData, nameZh: e.target.value })} className="rounded-2xl h-12 text-sm font-bold bg-muted/5 border border-slate-200 admin-interface-dark:border-white/10 shadow-inner" />
                  </div>
                  <div className="space-y-2.5">
                    <Label className="text-[9px] font-bold text-muted-foreground/30 uppercase pl-1 tracking-widest">中文简述 (ZH-DESC)</Label>
                    <Input placeholder="极致性能，为专业办公而生" value={formData.descZh} onChange={e => setFormData({ ...formData, descZh: e.target.value })} className="rounded-2xl h-12 text-[11px] font-bold bg-muted/5 border border-slate-200 admin-interface-dark:border-white/10 shadow-inner" />
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="space-y-2.5">
                    <Label className="text-[9px] font-bold text-muted-foreground/30 uppercase pl-1 tracking-widest">English Name (EN)</Label>
                    <Input placeholder="High Performance AIO" value={formData.nameEn} onChange={e => setFormData({ ...formData, nameEn: e.target.value })} className="rounded-2xl h-12 text-sm font-bold bg-muted/5 border border-dashed border-primary/20 focus:border-primary/40 admin-interface-dark:border-primary/20 shadow-inner" />
                  </div>
                  <div className="space-y-2.5">
                    <Label className="text-[9px] font-bold text-muted-foreground/30 uppercase pl-1 tracking-widest">English Desc (EN-DESC)</Label>
                    <Input placeholder="Professional performance for modern workspace" value={formData.descEn} onChange={e => setFormData({ ...formData, descEn: e.target.value })} className="rounded-2xl h-12 text-[11px] font-bold bg-muted/5 border border-dashed border-primary/20 focus:border-primary/40 admin-interface-dark:border-primary/20 shadow-inner" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-6 border-t border-border/40 admin-interface-dark:border-border/5">
              ...
              <div
                className="relative aspect-[8/1.5] rounded-2xl bg-muted/5 border-2 border-dashed border-slate-200 admin-interface-dark:border-white/10 overflow-hidden flex flex-col items-center justify-center group cursor-pointer hover:bg-primary/[0.02] hover:border-primary/40 transition-all shadow-inner"
              ...
              >
                {formData.thumbnailImageUrl ? (
                  ...
                  <div className="flex flex-col items-center gap-4 text-muted-foreground/40 admin-interface-dark:text-muted-foreground/20 group-hover:text-primary transition-colors">
                    <div className="h-12 w-12 rounded-xl bg-card shadow-md flex items-center justify-center border border-slate-200 admin-interface-dark:border-white/5">
                      <ImageIcon className="h-5 w-5" />
                    </div>
                    <span className="text-[10px] font-bold uppercase tracking-[0.3em]">从素材库选择媒体资源</span>
                  </div>
                )}
              </div>
            </div>
