    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="h-10 rounded-xl bg-muted/10 border-border/20 text-foreground"
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="h-10 rounded-xl bg-muted/10 border-border/20 border-dashed text-foreground"
                />
              </div>
              <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          <div className="col-span-full py-20 text-center border-2 border-dashed rounded-3xl bg-muted/5 border-border/20">
            ...
            <div key={item.id} className="group relative bg-card rounded-3xl border border-border/40 overflow-hidden shadow-sm hover:shadow-xl hover:translate-y-[-4px] transition-all duration-500">
              <div className="relative h-48 bg-muted/20">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  ...
                  {!item.published && (
                    <Badge variant="outline" className="bg-card/90 text-red-500 border-red-500/20 text-[10px] uppercase font-bold tracking-widest px-2.5 py-1 backdrop-blur-md">
                      <EyeOff className="h-3 w-3 mr-1" /> 已隐藏
                    </Badge>
                  )}
                </div>
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                   <div className="flex gap-2 bg-card/40 backdrop-blur-md p-2 rounded-full border border-white/10">
                     <Button variant="secondary" size="icon" className="h-8 w-8 rounded-full bg-white/10 hover:bg-white/20 text-white border-none" disabled={idx === 0} onClick={() => handleMove(idx, 'up')}><MoveUp className="h-4 w-4" /></Button>
                     <Button variant="secondary" size="icon" className="h-8 w-8 rounded-full bg-white/10 hover:bg-white/20 text-white border-none" disabled={idx === cases.length - 1} onClick={() => handleMove(idx, 'down')}><MoveDown className="h-4 w-4" /></Button>
                     <Button variant="secondary" size="icon" className="h-8 w-8 rounded-full bg-white/10 hover:bg-white/20 text-white border-none" onClick={() => handleOpenDialog(item)}><Edit2 className="h-4 w-4" /></Button>
              ...
              <div className="p-6 space-y-3">
                <h4 className="text-lg font-bold text-foreground truncate leading-tight group-hover:text-primary transition-colors">{item.titleZh}</h4>
                <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">{item.descZh}</p>
                <div className="pt-3 border-t border-border/10 border-dashed flex justify-between items-center">
                   <span className="text-[10px] font-mono text-muted-foreground/40 uppercase">SORT ORDER: {item.order}</span>
                   {item.titleEn ? <Badge variant="outline" className="h-4 px-1.5 text-[8px] border-green-500/20 text-green-500 bg-green-500/10">EN READY</Badge> : <Badge variant="outline" className="h-4 px-1.5 text-[8px] border-border/20 text-muted-foreground/40">NO EN</Badge>}
                </div>
              </div>
            </div>
          ))
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-5xl p-0 rounded-3xl overflow-hidden border-none shadow-2xl">
          ...
          <div className="p-8 bg-card grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={form.tagZh} onChange={e => setForm({...form, tagZh: e.target.value})} className="h-10 rounded-xl bg-muted/10 border-border/20 text-foreground" placeholder="如: 智慧零售" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase opacity-40">案例标题 (ZH)</Label>
                    <Input value={form.titleZh} onChange={e => setForm({...form, titleZh: e.target.value})} className="h-10 rounded-xl bg-muted/10 border-border/20 text-foreground" placeholder="如: 新加坡智慧零售转型" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase opacity-40">核心介绍 (ZH)</Label>
                  <Textarea value={form.descZh} onChange={e => setForm({...form, descZh: e.target.value})} className="min-h-[120px] rounded-xl bg-muted/10 border-border/20 text-foreground" placeholder="说明该方案如何解决客户痛点..." />
                </div>
              </div>
                ...
                >
                  {form.imageUrl ? (
                    ...
                    <div className="flex flex-col items-center gap-2 opacity-40">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest text-foreground">从云端素材库导入</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="flex justify-end border-l border-border/20 pl-8">
                {aiConfig?.isEnabled && (
                  <ShinyButton 
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-bold uppercase tracking-widest text-foreground">AI 智译右侧信息</span>
                    </div>
                  </ShinyButton>
                )}
              </div>

              <div className="space-y-4 pt-4 border-l pl-8 border-dashed border-border/20 flex-1">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase opacity-40 tracking-widest">TAG (EN)</Label>
                    <Input value={form.tagEn} onChange={e => setForm({...form, tagEn: e.target.value})} className="h-10 rounded-xl bg-muted/10 border-border/20 border-dashed text-foreground" placeholder="SMART RETAIL" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase opacity-40 tracking-widest">TITLE (EN)</Label>
                    <Input value={form.titleEn} onChange={e => setForm({...form, titleEn: e.target.value})} className="h-10 rounded-xl bg-muted/10 border-border/20 border-dashed text-foreground" placeholder="PROJECT TITLE" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase opacity-40 tracking-widest">DESCRIPTION (EN)</Label>
                  <Textarea value={form.descEn} onChange={e => setForm({...form, descEn: e.target.value})} className="min-h-[120px] rounded-xl bg-muted/10 border-border/20 border-dashed text-foreground" placeholder="DETAILED CASE DESCRIPTION..." />
                </div>
              </div>
              
                <div className="space-y-4 pt-6 border-l pl-8 border-dashed border-border/20">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase opacity-40">显示排序权重</Label>
                    <Input type="number" value={form.order} onChange={e => setForm({...form, order: parseInt(e.target.value)})} className="h-10 rounded-xl bg-muted/10 border-transparent font-mono text-foreground" />
                  </div>
                  
                  <div className="flex items-center justify-between p-4 bg-muted/10 rounded-2xl border border-dashed border-border/20">
                    <div className="space-y-0.5">
                      <Label className="text-xs font-bold uppercase tracking-tight text-foreground">前台显示状态</Label>
          ...
          <DialogFooter className="bg-muted/20 p-6 flex gap-3 border-t border-border/20">
