...
export default function UserManagementPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-3xl font-headline font-bold text-slate-900">成员与权限管理</h2>
          </div>
          <p className="text-sm text-slate-500 font-medium max-w-2xl pl-1">配置团队成员访问权限，确保业务数据安全。支持基于角色的权限包 (RBAC) 与原子化权限微调。</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <Input 
              ...
              className="pl-11 w-80 h-12 rounded-2xl border-slate-200 bg-white/50 backdrop-blur-sm focus:ring-primary/20"
              value={searchQuery}
          ...
          <Button 
            ...
            className="rounded-full h-12 px-8 font-bold uppercase tracking-widest text-xs gap-2 shadow-xl shadow-primary/20 bg-slate-900 hover:bg-slate-800"
          >
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] border border-white/40 shadow-2xl overflow-hidden">
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow className="hover:bg-transparent border-slate-100">
              <TableHead className="pl-8 py-5 font-bold uppercase text-[10px] tracking-widest text-slate-400">成员信息 / Profile</TableHead>
              <TableHead className="py-5 font-bold uppercase text-[10px] tracking-widest text-slate-400">系统角色 / Role</TableHead>
              <TableHead className="py-5 font-bold uppercase text-[10px] tracking-widest text-slate-400">活跃权限位 / Permissions</TableHead>
              <TableHead className="py-5 font-bold uppercase text-[10px] tracking-widest text-slate-400 text-right pr-8">操作 / Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={4} className="h-40 text-center text-slate-400 italic font-medium">未找到符合条件的成员</TableCell></TableRow>
            ) : filteredUsers.map((user) => (
              <TableRow key={user.id} className="group hover:bg-slate-50/80 transition-all border-slate-100">
                <TableCell className="pl-8 py-5">
                  <div className="flex items-center gap-4">
                    <Avatar className="h-12 w-12 rounded-2xl border-2 border-white shadow-md ring-1 ring-slate-100">
                    ...
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-900">{user.name || '未命名'}</span>
                ...
                <TableCell>
                  <div className="flex flex-wrap gap-1.5 max-w-md">
                    {user.role === 'superadmin' ? (
                      <Badge variant="outline" className="bg-amber-50 border-amber-200 text-amber-700 text-[9px] font-bold uppercase tracking-tighter">Full System Access</Badge>
                    ) : user.permissions?.length > 0 ? (
                      user.permissions.slice(0, 3).map(p => (
                        <Badge key={p} variant="outline" className="bg-slate-50 border-slate-200 text-slate-500 text-[9px] font-bold uppercase tracking-tighter px-2">
                          {p.replace('_', ' ')}
                        </Badge>
                      ))
                    ) : (
                      <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest italic">None Assigned</span>
                    )}
                    {user.role !== 'superadmin' && user.permissions?.length > 3 && (
                      <Badge variant="outline" className="bg-slate-50 border-slate-200 text-slate-400 text-[9px] font-bold px-2">+{user.permissions.length - 3}</Badge>
                    )}
                  </div>
                </TableCell>
                <TableCell className="text-right pr-8">
                  <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    {user.role === 'superadmin' && (
                      <div className="h-10 w-10 flex items-center justify-center text-slate-200">
      ...
      <Dialog open={!!editingUser} onOpenChange={(open) => !open && setEditingUser(null)}>
        <DialogContent className="max-w-2xl p-0 overflow-hidden border-none shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] rounded-[2.5rem] bg-white">
          ...
          <div className="p-8 space-y-8 max-h-[60vh] overflow-y-auto scrollbar-minimal">
            ...
            <div className="space-y-4">
              <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">系统角色定义 (Core Role)</Label>
              <div className="grid grid-cols-2 gap-4">
                <div 
                  onClick={() => setSelectedRole('superadmin')}
                  className={cn(
                    "p-5 rounded-3xl border-2 transition-all cursor-pointer group flex flex-col gap-2",
                    selectedRole === 'superadmin' ? "border-primary bg-primary/5 ring-4 ring-primary/5" : "border-slate-100 hover:border-slate-200"
                  )}
                >
                  <div className="flex items-center justify-between">
                    <ShieldCheck className={cn("h-6 w-6", selectedRole === 'superadmin' ? "text-primary" : "text-slate-300")} />
                  ...
                  <p className="text-[10px] text-slate-400 leading-relaxed font-medium">拥有系统最高权限，包括所有业务模块和成员管理。</p>
                </div>
                ...
                <div 
                  ...
                  className={cn(
                    "p-5 rounded-3xl border-2 transition-all cursor-pointer group flex flex-col gap-2",
                    selectedRole === 'editor' ? "border-primary bg-primary/5 ring-4 ring-primary/5" : "border-slate-100 hover:border-slate-200"
                  )}
                >
                  <div className="flex items-center justify-between">
                    <Lock className={cn("h-6 w-6", selectedRole === 'editor' ? "text-primary" : "text-slate-300")} />
                    {selectedRole === 'editor' && <CheckCircle2 className="h-5 w-5 text-primary" />}
                  </div>
                  <span className="font-bold text-slate-900">General Editor</span>
                  <p className="text-[10px] text-slate-400 leading-relaxed font-medium">受限角色，仅能访问被显式授权的业务模块。</p>
                </div>
              </div>
            </div>
            ...
            {selectedRole === 'editor' && (
              <div className="space-y-6 animate-in slide-in-from-top-4 duration-500">
                <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">细分权限分配 (Granular Permissions)</Label>
                <div className="space-y-4">
                  {PERMISSION_GROUPS.map(group => (
                    <div key={group.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-4 transition-all hover:bg-white hover:shadow-xl hover:shadow-slate-200/50">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                      ...
                      <div className="grid grid-cols-2 gap-3 pl-12">
                        {group.items.map(perm => (
                          <div key={perm} className="flex items-center space-x-3 group/perm">
                            ...
                            />
                            <label htmlFor={perm} className="text-[11px] font-bold text-slate-500 cursor-pointer group-hover/perm:text-slate-900 transition-colors uppercase tracking-tight">
                              {perm.replace('_', ' ')}
                            </label>
                          </div>
                        ))}
            ...
            {selectedRole === 'superadmin' && (
              <div className="p-8 bg-amber-50 rounded-3xl border border-amber-200 flex gap-5 animate-in zoom-in-95 duration-500">
                <ShieldAlert className="h-10 w-10 text-amber-500 shrink-0" />
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-amber-900 uppercase tracking-wider">最高权限预警</h4>
                  <p className="text-[11px] text-amber-700 leading-relaxed font-medium">超级管理员将自动获得全站所有模块的读写权限，包括敏感的系统配置、成员管理及财务数据。请确保账号归属人具备最高安全信任度。</p>
                </div>
              </div>
            )}
          </div>

          <DialogFooter className="bg-slate-50 p-8 border-t border-slate-100 gap-4">
            <Button variant="ghost" onClick={() => setEditingUser(null)} className="h-14 rounded-2xl flex-1 font-bold uppercase tracking-widest text-[10px] text-slate-400">放弃修改</Button>
            <Button 
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] rounded-[2rem] bg-white">
          ...
          <div className="p-8 space-y-6">
            <div className="space-y-4">
               <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest">成员姓名</Label>
                  <Input 
               ...
               <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest">登录邮箱</Label>
                  <Input 
                    type="email"
               ...
               <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest">初始密码</Label>
                  <Input 
                    type="password"
                    placeholder="••••••••" 
                    className="h-12 rounded-xl bg-slate-50 border-slate-100"
               ...
               <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest">预设角色</Label>
                  <div className="grid grid-cols-2 gap-3">
                    <button 
                      onClick={() => setNewUser(prev => ({ ...prev, role: 'editor' }))}
                      className={cn(
                        "py-3 rounded-xl border-2 text-xs font-bold transition-all",
                        newUser.role === 'editor' ? "border-primary bg-primary/5 text-primary" : "border-slate-100 text-slate-400 hover:border-slate-200"
                      )}
                    >
                      普通编辑 (Editor)
                    </button>
                    <button 
                      ...
                      className={cn(
                        "py-3 rounded-xl border-2 text-xs font-bold transition-all",
                        newUser.role === 'superadmin' ? "border-primary bg-primary/5 text-primary" : "border-slate-100 text-slate-400 hover:border-slate-200"
                      )}
                    >
                      超级管理 (Admin)
                    </button>
          ...
          <DialogFooter className="p-8 bg-slate-50 border-t border-slate-100 gap-3">
             <Button variant="ghost" onClick={() => setIsCreateDialogOpen(false)} className="h-12 rounded-xl flex-1 font-bold uppercase text-[10px] tracking-widest text-slate-400">取消</Button>
             <Button 
               onClick={handleCreateUser}
               disabled={isUpdating}
               className="h-12 rounded-xl flex-1 font-bold uppercase text-[10px] tracking-widest bg-slate-900 shadow-xl shadow-slate-200"
             >
