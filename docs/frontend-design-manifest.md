...
*   **结构定义**: 采用 `bg-muted/20` 或 `bg-primary/5` 承托底座，内嵌 `p-1` 间距。
*   **状态切换**: 激活项采用实色背景+纯白文字；非激活项在悬停时采用 `bg-primary/10` 或品牌色转换反馈。
*   **下载专用样式**: 规格书/手册下载类按钮，采用 `rounded-2xl`，背景 `bg-primary/5`，图标透明度 `40%`，悬浮时全反转为品牌实色。
...
### 12.3 核心应用场景
*   **全局导航**: 顶部固定导航栏激活态强制应用 **Level 02: Frosted** 级别。
*   **材质参数**: 12px 模糊半径，60% 白色透明度，配合 1px 白色描边。
...
### 16.2 顶栏布局与巨型菜单 (Navbar & Mega Menu)
*   **布局对齐**: 采用 **非对称右对齐** 逻辑。Logo 固定在左侧，一级菜单项与功能区（语言切换、Contact）通过 `ml-auto` 统一推向右侧，形成视觉重心偏移。
*   **巨型菜单结构**: 采用 `2.5rem` 圆角的玻璃面板，内部分为 3-4 列展示产品线。
*   **主推卡片 (Featured)**: 
    *   **极简原则**: 卡片仅保留核心多媒体展示与操作按钮，移除多余的标题与描述文字。
    *   **容器样式**: 移除投影 (`shadow`)，使其平滑融入二级菜单背景，减少视觉干扰。
*   **交互反馈**: 悬停时触发 `Gentle` 曲线的平滑位移。
